...
describe SavonZuora::Client do
  let(:client) { SavonZuora::Client.new { wsdl.document = Endpoint.wsdl } }

  describe ".new" do
    context "with a String" do
      it "should set the WSDL document" do
        wsdl = "http://example.com/UserService?wsdl"
        client = SavonZuora::Client.new(wsdl)
    ...
    context "with a block expecting one argument" do
      it "should yield the WSDL object" do
        SavonZuora::Client.new { |wsdl| wsdl.should be_a(Wasabi::Document) }
      end
    end

    context "with a block expecting two arguments" do
      it "should yield the WSDL and HTTP objects" do
        SavonZuora::Client.new do |wsdl, http|
    ...
    context "with a block expecting three arguments" do
      it "should yield the WSDL, HTTP and WSSE objects" do
        SavonZuora::Client.new do |wsdl, http, wsse|
          wsdl.should be_an(Wasabi::Document)
          http.should be_an(HTTPI::Request)
    ...
    context "with a block expecting no arguments" do
      it "should let you access the WSDL object" do
        SavonZuora::Client.new { wsdl.should be_a(Wasabi::Document) }
      end

      it "should let you access the HTTP object" do
        SavonZuora::Client.new { http.should be_an(HTTPI::Request) }
      end

      it "should let you access the WSSE object" do
        SavonZuora::Client.new { wsse.should be_a(Akami::WSSE) }
      end
    end
  end

  describe "#wsdl" do
    it "should return the SavonZuora::Wasabi::Document" do
  ...
  describe "#request" do
    ...
    before do
      ...
      SavonZuora::SOAP::RequestBuilder.stubs(:new).returns(request_builder)
    ...
    describe "setting dependencies of the request builder" do
      ...
      it "sets the config property with a SavonZuora::Config object" do
        request_builder.expects(:config=).with { |config| config.is_a?(SavonZuora::Config) }
        client.request(:get_user)
      end
    end
    ...
    context "with two Symbols" do
      ...
      it "should not set the target namespace if soap.namespace was set to nil in the post-configuration block" do
        SavonZuora::SOAP::RequestBuilder.unstub(:new)
    ...
    context "with a block" do
      ...
      it "passes the block to the request builder" do
        ...
        SavonZuora::SOAP::RequestBuilder.stubs(:new).returns(dummy)
      ...
      it "executes the block in the context of the request builder" do
        SavonZuora::SOAP::RequestBuilder.class_eval do
          def who
            self
          end
        end

        client.request(:authenticate) { who.should be_a SavonZuora::SOAP::RequestBuilder }
      end

      context "with a block expecting one argument" do
        it "should yield the SOAP object" do
          client.request(:authenticate) { |soap| soap.should be_a SavonZuora::SOAP::XML }
        end
      end
      ...
      context "with a block expecting two arguments" do
        it "should yield the SOAP and WSDL objects" do
          client.request(:authenticate) do |soap, wsdl|
            soap.should be_a(SavonZuora::SOAP::XML)
            wsdl.should be_an(Wasabi::Document)
          end
        end
      end
      ...
      context "with a block expecting no arguments" do
        it "should let you access the SOAP object" do
          client.request(:authenticate) { soap.should be_a(SavonZuora::SOAP::XML) }
        end
  ...
  context "with a local WSDL document" do
    let(:client) { SavonZuora::Client.new { wsdl.document = "spec/fixtures/wsdl/authentication.xml" } }
  ...
  context "with an Array of namespaced items" do
    context "with a single namespace" do
      let(:client) { SavonZuora::Client.new { wsdl.document = "spec/fixtures/wsdl/taxcloud.xml" } }
    ...
    context "with multiple namespaces" do
      let(:client) { SavonZuora::Client.new { wsdl.document = "spec/fixtures/wsdl/multiple_namespaces.xml" } }
  ...
  context "when encountering a SOAP fault" do
    ...
    it "should raise a SavonZuora::SOAP::Fault" do
      lambda { client.request :authenticate }.should raise_error(SavonZuora::SOAP::Fault)
    end
  end
  ...
  context "when encountering an HTTP error" do
    ...
    it "should raise a SavonZuora::HTTP::Error" do
      lambda { client.request :authenticate }.should raise_error(SavonZuora::HTTP::Error)
    end
  end
  ...
  def expect_request_builder_to_receive(operation, options = {})
    SavonZuora::SOAP::RequestBuilder.expects(:new).with(operation, options).returns(request_builder)
  end
