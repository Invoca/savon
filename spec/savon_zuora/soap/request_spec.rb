...
describe SavonZuora::SOAP::Request do
  ...
  def soap_xml!(endpoint = nil, input = nil, body = nil)
    soap = SavonZuora::SOAP::XML.new(config)
  ...
  it "contains the content type for each supported SOAP version" do
    content_type = SavonZuora::SOAP::Request::CONTENT_TYPE
  ...
  describe ".execute" do
    it "executes a SOAP request and returns the response" do
      ...
      response = SavonZuora::SOAP::Request.execute config, http_request, soap_xml
  ...
  describe ".new" do
    ...
    it "sets the Content-Type header for SOAP 1.1" do
      soap_request.http.headers["Content-Type"].should == SavonZuora::SOAP::Request::CONTENT_TYPE[1]
    end

    it "sets the Content-Type header for SOAP 1.2" do
      soap_xml.version = 2
      soap_request.http.headers["Content-Type"].should == SavonZuora::SOAP::Request::CONTENT_TYPE[2]
    end
    ...
    it "sets the Content-Length header for every request" do
      ...
      soap_request = SavonZuora::SOAP::Request.new(config, http, soap_xml)
    ...
    context "with a :soap_request hook" do
      it "lets you replace the HTTP request and return your own response" do
        config.hooks.define(:test, :soap_request) do |_, request|
          request.should be_a(SavonZuora::SOAP::Request)
          http_response
        end
