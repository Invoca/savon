...
describe SavonZuora::SOAP::Response do

  describe ".new" do
    it "should raise a SavonZuora::SOAP::Fault in case of a SOAP fault" do
      lambda { soap_fault_response }.should raise_error(SavonZuora::SOAP::Fault)
    end

    it "should not raise a SavonZuora::SOAP::Fault in case the default is turned off" do
      SavonZuora.raise_errors = false
      lambda { soap_fault_response }.should_not raise_error(SavonZuora::SOAP::Fault)
      SavonZuora.raise_errors = true
    end

    it "should raise a SavonZuora::HTTP::Error in case of an HTTP error" do
      lambda { soap_response :code => 500 }.should raise_error(SavonZuora::HTTP::Error)
    end

    it "should not raise a SavonZuora::HTTP::Error in case the default is turned off" do
      SavonZuora.raise_errors = false
  ...
  describe "#soap_fault" do
    ...
    it "should return a SavonZuora::SOAP::Fault" do
      soap_fault_response.soap_fault.should be_a(SavonZuora::SOAP::Fault)
    end

    it "should return a SavonZuora::SOAP::Fault containing the HTTPI::Response" do
      soap_fault_response.soap_fault.http.should be_an(HTTPI::Response)
    end

    it "should return a SavonZuora::SOAP::Fault even if the SOAP response seems to be ok" do
      soap_response.soap_fault.should be_a(SavonZuora::SOAP::Fault)
    end
  end
  ...
  describe "#http_error" do
    ...
    it "should return a SavonZuora::HTTP::Error" do
      http_error_response.http_error.should be_a(SavonZuora::HTTP::Error)
    end

    it "should return a SavonZuora::HTTP::Error containing the HTTPI::Response" do
      http_error_response.http_error.http.should be_an(HTTPI::Response)
    end

    it "should return a SavonZuora::HTTP::Error even if the HTTP response seems to be ok" do
      soap_response.http_error.should be_a(SavonZuora::HTTP::Error)
    end
  end
  ...
  describe "#[]" do
    ...
    it "should throw an exception when the response body isn't parsable" do
      lambda { invalid_soap_response.body }.should raise_error SavonZuora::SOAP::InvalidResponseError
    end
  end
  ...
  describe "#header" do
    ...
    it "should throw an exception when the response header isn't parsable" do
      lambda { invalid_soap_response.header }.should raise_error SavonZuora::SOAP::InvalidResponseError
    end
  end
  ...
  def soap_response(options = {})
    ...
    SavonZuora::SOAP::Response.new HTTPI::Response.new(response[:code], response[:headers], response[:body])
  end
