...
describe SavonZuora::SOAP::Fault do
  let(:soap_fault) { SavonZuora::SOAP::Fault.new new_response(:body => Fixture.response(:soap_fault)) }
  let(:soap_fault2) { SavonZuora::SOAP::Fault.new new_response(:body => Fixture.response(:soap_fault12)) }
  let(:another_soap_fault) { SavonZuora::SOAP::Fault.new new_response(:body => Fixture.response(:another_soap_fault)) }
  let(:no_fault) { SavonZuora::SOAP::Fault.new new_response }

  it "should be a SavonZuora::Error" do
    SavonZuora::SOAP::Fault.should < SavonZuora::Error
  end
