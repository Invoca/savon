...
describe SavonZuora::SOAP::XML do
  let(:xml) { SavonZuora::SOAP::XML.new Endpoint.soap, [nil, :authenticate, {}], :id => 1 }

  describe ".new" do
    it "should accept an endpoint, an input tag and a SOAP body" do
      xml = SavonZuora::SOAP::XML.new Endpoint.soap, [nil, :authentication, {}], :id => 1
  ...
  describe "#header" do
    ...
    it "should use the global soap_header if set" do
      SavonZuora.stubs(:soap_header).returns({ "MySecret" => "abc" })
      xml.header.should == { "MySecret" => "abc" }
    end
  end
  ...
  describe "#env_namespace" do
    ...
    it "should use the global env_namespace if set as the SOAP envelope namespace" do
      SavonZuora.stubs(:env_namespace).returns(:soapenv)
      xml.env_namespace.should == :soapenv
    end
  end
  ...
  describe "#to_xml" do
    ...
    context "with :element_form_default set to :qualified and a :namespace" do
      let :xml do
        SavonZuora::SOAP::XML.new Endpoint.soap, [nil, :authenticate, {}], :user => { :id => 1, ":noNamespace" => true }
      end
  ...
  def reset_soap_version
    SavonZuora.soap_version = SavonZuora::SOAP::DefaultVersion
  end
