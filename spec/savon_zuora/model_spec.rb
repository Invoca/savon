...
describe SavonZuora::Model do

  let(:model) do
    Class.new { extend SavonZuora::Model }
  end
  ...
  describe ":model_soap_response hook" do
    ...
    after do
      SavonZuora.hooks.reject! :test_hook
    end

    it "can be used for pre-processing SOAP responses" do
      SavonZuora.hooks.define(:test_hook, :model_soap_response) do |response|
        "hello #{response}"
      end
  ...
  describe ".client" do
    ...
    it "passes a given block to a new SavonZuora::Client"

    it "memoizes the SavonZuora::Client" do
  ...
  describe "#client" do
    ...
    it "returns the class-level SavonZuora::Client" do
