...
describe SavonZuora::Wasabi::Document do
  ...
  context "with a remote document" do
    ...
    it "should resolve via HTTP" do
      wsdl = SavonZuora::Wasabi::Document.new("http://example.com?wsdl")
      wsdl.xml.should == Fixture.wsdl(:authentication)
    end

    it "should resolve via HTTPS" do
      wsdl = SavonZuora::Wasabi::Document.new("https://example.com?wsdl")
      wsdl.xml.should == Fixture.wsdl(:authentication)
    end
  ...
  context "with an inaccessible remote document" do
    ...
    it "should raise an error when authentication fails" do
      wsdl = SavonZuora::Wasabi::Document.new("http://example.com?wsdl")
      expect { wsdl.xml }.to raise_error(SavonZuora::HTTP::Error)
    end
  end
  ...
  context "with a local document" do
    ...
    it "should read the file" do
      wsdl = SavonZuora::Wasabi::Document.new("spec/fixtures/wsdl/authentication.xml")
      wsdl.xml.should == Fixture.wsdl(:authentication)
    end
  end
  ...
  context "with raw XML" do
    ...
    it "should use the raw XML" do
      wsdl = SavonZuora::Wasabi::Document.new Fixture.wsdl(:authentication)
      wsdl.xml.should == Fixture.wsdl(:authentication)
    end
  end
